...
| `local-review` | `--diff FILE` (default stdin); optional `--model`, `--base-url`, `--max-chars`, `--timeout` (default from `[pr_automation.fallback]`) | Review a diff with a local Ollama-compatible model when the primary paid review returned no verdict at all. Assesses only `pass`/`summary`/`findings`; every documentation-contract field is reported `true` but explicitly marked unevaluated in the summary. Fails closed (nonzero exit) on an empty diff, an unreachable model, or an unusable response. Runs only from the `review-fallback` job on a self-hosted runner; not part of the five-surface capability registry. |
