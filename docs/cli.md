...
| `local-review` | `--diff FILE` (default stdin); optional `--model`, `--base-url`, `--max-chars`, `--timeout` (default from `[pr_automation.fallback]`) | Review a diff with a local Ollama-compatible model when the primary exact-head review returned no verdict at all. Assesses only `pass`/`summary`/`findings`; every documentation-contract field is reported `true` but explicitly marked unevaluated in the summary. Fails closed (nonzero exit) on an empty diff, an unreachable model, or an unusable response. Runs only from the `review-fallback` job on a self-hosted runner; not part of the five-surface capability registry. |
| `local-triage` | `--issue FILE` (default stdin); optional `--model`, `--base-url`, `--max-chars`, `--timeout` (default from `[pr_automation.fallback]`) | Triage an issue with the same local model when the paid solver produced no attempt at all. Never writes code; emits a bounded root-cause/approach/files/risks analysis for a comment and always forces `needs_human=true`. Fails closed on empty input or an unusable model response. |
